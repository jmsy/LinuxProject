...
#endif
extern void *sdma_alloc_coherent(struct device *dev, size_t size, dma_addr_t *dma_handle,gfp_t gfp);
extern void sdma_free_coherent(struct device *dev,size_t size,void *vaddr,dma_addr_t bus);
...
static int rtl8139_open (struct net_device *dev)
{
	...
	tp->tx_bufs = sdma_alloc_coherent(&tp->pci_dev->dev, TX_BUF_TOT_LEN,
					   &tp->tx_bufs_dma, GFP_KERNEL);
	tp->rx_ring = sdma_alloc_coherent(&tp->pci_dev->dev, RX_BUF_TOT_LEN,
					   &tp->rx_ring_dma, GFP_KERNEL);
	if (tp->tx_bufs == NULL || tp->rx_ring == NULL) {
		free_irq(dev->irq, dev);

		if (tp->tx_bufs)
			sdma_free_coherent(&tp->pci_dev->dev, TX_BUF_TOT_LEN,
					    tp->tx_bufs, tp->tx_bufs_dma);
		if (tp->rx_ring)
			sdma_free_coherent(&tp->pci_dev->dev, RX_BUF_TOT_LEN,
	...
	int i;
//	int *hacker=(int *)tp->tx_bufs-1;
	int *hacker1=(int *)(tp->tx_bufs+TX_BUF_TOT_LEN);
	printk("cal_address %p\n",hacker1);
	for(i=0;i<1;i++)
	{
	//  hacker[i]=i+1;
	  hacker1[i]=i+1;
